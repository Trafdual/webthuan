...
function renderCalendar(date) {
  ...
  for (let day = 1; day <= lastDateOfMonth; day++) {
    ...
    div.addEventListener("click", () => selectDate(new Date(dateString), div));
    calendarDates.appendChild(div);
  }
...
function selectDate(date, cell) {
  ...
  const start = selectedStartDate ? selectedStartDate.toISOString().split("T")[0] : "";
  const end = selectedEndDate ? selectedEndDate.toISOString().split("T")[0] : "";
...
dateInput.addEventListener("click", () => {
  calendarContainer.style.display =
    calendarContainer.style.display === "none" || !calendarContainer.style.display
      ? "block"
      : "none";
});
